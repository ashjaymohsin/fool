...
2. **Auto-search for thread link (comments only, all platforms):**
   If the entry is a **comment** (not a post) and the user has NOT provided a `post_link`, check if the comment text references a specific thread, topic, video, or discussion. If it does:
   - Use WebSearch to find the real URL of that thread/video/discussion on the relevant platform
   - Use the most relevant result as `post_link`
   - If no confident match is found, leave `post_link` empty and mention it to the user
   - Do NOT auto-search for posts — only comments need this

3. Convert to JSON and run the script:

**Single entry:**
```powershell
python "$env:USERPROFILE\.claude\write_sheet.py" '{"platform": "reddit", "comment": "...", "title": "...", "post_link": "..."}'
```

**Multiple entries (batch):**
```powershell
python "$env:USERPROFILE\.claude\write_sheet.py" '[{"platform": "reddit", "comment": "..."}, {"platform": "youtube", "comment": "..."}]'
```
...
- For auto-search: prefer direct links to the specific thread/video/post, not just the platform homepage or subreddit/channel page
